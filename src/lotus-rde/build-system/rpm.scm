...
(define-module (lotus-rde build-system rpm)
  #:use-module ((lotus-rde build rpm-build-system))
...
(define %rpm-build-system-modules
  ...
  `((lotus-rde build rpm-build-system)
...
(define* (rpm-build name inputs
                    ...
                    ;;             %standard-phases))
                    (phases '(@ (lotus-rde build rpm-build-system)
                                %standard-phases))
                    (input-lib-mapping ''())
                    (readonly-binaries '#f)
                    (locale "en_US.utf8")
                    ...
                    (modules '((lotus-rde build rpm-build-system)
                               (guix build utils)))
                    (substitutable? #t)
                    allowed-references
