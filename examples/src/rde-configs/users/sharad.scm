    ...
    (list
     (rde-elisp-configuration-service
      ...
      `((with-eval-after-load 'org
        ...
        ,@(@ (rde-configs users sharad emacs) telega-config)

        (with-eval-after-load 'geiser-mode
          (setq geiser-mode-auto-p nil)
          (defun sharad-geiser-connect ()
            (interactive)
            (geiser-connect 'guile "localhost" "37146"))

          (define-key geiser-mode-map (kbd "C-c M-j") 'sharad-geiser-connect))
...
(define-public emacs-arei-local
  (package
    ...
    (version "0.9.99999")
    (source
     (local-file "/data/sharad/work/sharad/emacs-arei" #:recursive? #t))))
...
(define-public guile-ares-rs-local
  (package
    ...
    (version "0.9.99999")
    (source
     (local-file "/data/sharad/work/sharad/guile-ares-rs" #:recursive? #t))))
...
(define rde-guix-add-to-shell-profile-service
  (simple-service
   ...
   (list "
GUIX_PROFILE=/data/sharad/work/sharad/rde/examples/target/profiles/guix
...
subject:/home:/) and tag:new}\"'"
    ...
    (feature-emacs-cua)
    (feature-difftastic #:parse-error-limit 1000)
    (feature-keyboard
